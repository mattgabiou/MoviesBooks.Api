 ...
 * Only exists once the EF Core package is installed.
 */
...
/*
 * An ENTITY MODEL: a C# class that EF Core maps to one table row.
 ...
 *
 * [Table] pins the class to the real table name. Without it EF would use the
 * DbSet name ("Movies") and fail with "Invalid object name". The C# side
 * stays clean (Movie / db.Movies); only the mapping knows the odd name.
 */
[Table("Movie_Name")]
public class Movie
{
     ...
     * If the column is ever changed to nvarchar, drop the attribute.
     */
    [MaxLength(100)]
    [Unicode(false)]
    ...
    /*
     * int, nullable -> int? (Nullable<int>, a real wrapper struct).
     * Same story for Rating and Lost below.
     */
